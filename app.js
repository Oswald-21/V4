...
function enlazarGeneradorTemporal() {
  ...
  const volverAlCatalogo = () => {
    document.body.classList.remove("generator-focus");
    elementos.panelGenerador.hidden = true;
    elementos.abrirGenerador.setAttribute("aria-expanded", "false");
    limpiar();
    elementos.abrirGenerador.focus();
  };
  ...
  elementos.formularioEAN.addEventListener("submit", (evento) => {
    ...
    // Al enviar desde el teclado móvil, retiramos el foco para cerrarlo.
    elementos.entradaEAN.blur();
    elementos.mensajeEAN.textContent = "Código temporal generado. No se ha guardado.";
    document.body.classList.add("generator-focus");
    elementos.productoTemporal.innerHTML = `
      ...
      <code>${ean}</code>
      <button type="button" class="close-temporary">Volver al catálogo</button>
    `;
